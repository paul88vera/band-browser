...
function getArtistInfo(artist) {
  ...
  fetch(bandInfoURL).then(function (response) {
    if (response.ok) {
...
//* GETS USER INPUT AND PLUGS INTO GETARTISTINFO FUNCTION */
...
//* FETCH TICKETMASTER DATES */
function getEventInfo(artist) {
  var tixAPI =
    "https://app.ticketmaster.com/discovery/v2/events.json?keyword=" +
    artist +
    "&apikey=GaTAMAKytFLtlNib0wnQuqnwmT0iMzXy";

  fetch(tixAPI).then(function (response) {
    response.json().then(function (data) {
...
//* DISPLAYS MODAL WHEN MAGNIFYING GLASS IS SELECTED */
...
//* FUNCTION TO CREATE AND DISPLAY MULTIPLE EVENTS */
var eventInfoDisplay = function (data) {
  ...
  bandEventContainerEl.innerHTML =
    "<a href='https://www.ticketmaster.com/'>View More Concerts...</a>";
};
